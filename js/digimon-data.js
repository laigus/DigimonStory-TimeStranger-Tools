// 根据数码宝贝名称生成图片路径的函数
function getDigimonImagePath(name) {
    // 首先尝试从 picture 目录加载图片
    // 支持的图片格式
    const imageExtensions = ['.jpg', '.jpeg', '.png', '.gif', '.webp'];
    
    // 尝试常见的图片格式
    for (const ext of imageExtensions) {
        const imagePath = `picture/${name}${ext}`;
        // 返回第一个匹配的路径（优先 jpg）
        if (ext === '.jpg') {
            return imagePath;
        }
    ...
    // 如果没有找到本地图片，返回占位符
    return `https://via.placeholder.com/80x80/CCCCCC/FFFFFF?text=${encodeURIComponent(name)}`;
}
